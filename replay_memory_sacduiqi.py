...
class ReplayMemory(object):
    def __init__(self, obs,max_size, obs_dim, act_dim):
        ...
        self.obs = np.zeros((max_size, obs,obs_dim), dtype='float32')
        if act_dim == 0:  # Discrete control environment
            self.action = np.zeros((max_size,obs ), dtype='int32')
        else:  # Continuous control environment
            self.action = np.zeros((max_size,obs, act_dim), dtype='float32')
        self.reward = np.zeros((max_size, ), dtype='float32')
        self.terminal = np.zeros((max_size, ), dtype='bool')
        self.next_obs = np.zeros((max_size,obs,obs_dim), dtype='float32')

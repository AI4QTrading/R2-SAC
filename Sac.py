...
class SAC(parl.Algorithm):
    ...
    def predict(self, obs):
        ...
        act_mean = (act_mean + paddle.to_tensor(np.random.normal(0, 1*40, size=act_mean.shape)))
    ...
    def _critic_learn(self, obs, action, reward, next_obs, terminal):
        ...
        cur_q1= cur_q1.sum(axis=1)
        cur_q2= cur_q2.sum(axis=1)
    ...
    def _actor_learn(self, obs):
        ...
        actor_loss = ((self.alpha * log_pi) - min_q_pi).sum(axis=1).mean()

        self.actor_optimizer.clear_grad()

        actor_loss.backward()
        #print(self.actor_optimizer.minimize(actor_loss)[1][1])
        self.actor_optimizer.step()
        #print(self.actor_optimizer.clear_gradients)
        return actor_loss
